...
public class MemberServiceImpl implements MemberService{
	...
	public void insertMember(Member member) throws Exception {
		...
		//데이터 저장처리 = 가입
		// not null로 설정된 값이 설정되지 않았다면 예외 발생함
		try {
	...
	public Member selectLoginInfo(Member member) throws Exception {
		...
		try {
			...
			//조회된 데이터가 없다는 것은 WHERE절 조건에 맞는 데이터가 없다는것
	...
	public void updateMemberPasswordByEmail(Member member) throws Exception {
		try {
			...
			sqlSession.rollback();
			throw new Exception ("가입된 이메일이아닙니다");
		} catch(Exception e) {
			sqlSession.rollback();
			logger.error(e.getLocalizedMessage());
	...
	public void selectMemberPasswordCount(Member member) throws Exception {
		try {
			...
			//회원번호와 비밀번호가 일치하는 데이터가 0이라면 비밀번호가 잘못된상태
			if(result == 0) {
				throw new NullPointerException();
			}
		} catch(NullPointerException e) {
	...
	public void deleteMember(Member member) throws Exception {
		try {
			...
			//삭제된 데이터가 없다는 것은 WHERE절의 조건값이 맞지 않는다는 의미
			// 이경우, 첫번째 WHERE조건에서 사용되는 id값에 대한 회원을 찾을 수 없다는 의미
			if(result == 0) {
				throw new NullPointerException();
			}
		} catch(NullPointerException e) {
